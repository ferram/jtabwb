...
class EnginePlain {
  ...
  _AbstractRule applyRule(_AbstractRule ruleToApply, RuleType ruleType) {
    ...
    if (LAST_ITERATION_INFO.move != Move.BACKTRACK_POINT_SEARCH
        && LAST_ITERATION_INFO.move != Move.BRANCH_POINT_SEARCH)
    ...
    // list of node TOP at the top of the stack.  
    // The status of ruleToApply will be notified when TOP or a node before T will be  
  ...
  ProofSearchResult searchProof() {
    ...
    while (!finished) {
      ...
      if (nextRule != null) {
        ...
        case FORCE_BRANCH_SUCCESS: {
          // update iteration info
          LAST_ITERATION_INFO.move = IterationInfo.Move.BRANCH_POINT_SEARCH;
          LAST_ITERATION_INFO.number_of_iterations++;
